...
public class Maquina extends Usuario{
	...
	/**
	 * Almacena una colección con los {@link #coloresCorrectos} junto a sus posiciones relativas con resecto a la combinacion a adivinar.
	 * Forma parte de la IA.
	 * @see Combinacion
	 * @see Color
	 */
	private TreeMap<Byte,Color> mapaColores;
	/**
	...
	private void crearMaquina() {
		...
		numCambiosFase2 = 0;
		primeraMitad = true;
		mapaColores = new TreeMap<Byte,Color>();
	...
	private void comprobarColores(byte fichasNoVacias,Ronda ronda) {
		...
		Combinacion combinacionByN = rondas.getLast().getResultadoByN();
		switch (dificultad) {
			case FACIL_ADIVINAR:
...
//=========================================================================================================================================================
	...
	private void comprobarPosiciones(boolean primeraParte) {
		...
		boolean apareceIzq;
		boolean yaAsignado;
		Color coloresIzq[] = {mapaColores.get((byte)0),mapaColores.get((byte)1)};
		Color coloresDer[] = {mapaColores.get((byte)2),mapaColores.get((byte)3)};
		Ronda ronda;
		...
		for (byte contador=0;contador<2;contador++) {	// Lo hacemos 2 veces para que compruebe tanto segun el primer color como el segundo y no repita una ronda
			...
			for (Color color:coloresCorrectos) {	// Se recorre el array de colores correctos
				if (primeraParte) {		// Si esta en la primera parte
								...
								if (contarFichasByN(combinacionByN)==1||combinacionByN.getCombinacion()[0].equals(combinacionByN.getCombinacion()[1])) {	// Cubre 2 del mismo color y 1 con nulo
									...
									for (Color c:coloresIzq) {	// Si el compañeo está en la izquierda
										if (c==ronda.getCombinacionPropuesta().getCombinacion()[0].getColorFicha()){
											mapaColores.put(contadorIzq,color);
									...
									if (!yaAsignado){	// Si no se ha asignado aún se comprueba esto
										for (Color c:coloresDer) {	// Si el compañero está en la derecha
											if (c==ronda.getCombinacionPropuesta().getCombinacion()[(dificultad.getNumCasillas()/2)-1].getColorFicha()){
												mapaColores.put(contadorDer,color);
					...
					if (!mapaColores.containsValue(color)) {	// Si aún no se ha asignado a ninguna posición se comprueba de qué posición es siempre que sea posible y si no se consiguen rellenar todas las posiciones, se hace otra ronda
						if (mapaColores.containsKey((byte)0)&&mapaColores.containsKey((byte)1)) {
							if (mapaColores.containsKey((byte)2))
								mapaColores.put((byte)3,color);
							else
								mapaColores.put((byte)2,color);
						}else if (mapaColores.containsKey((byte)2)&&mapaColores.containsKey((byte)3)) {
							if (mapaColores.containsKey((byte)0))
								mapaColores.put((byte)1,color);
							else
								mapaColores.put((byte)0,color);
						}
					}
				}
			}
			...
			switch (fase) {
				case 1:
					for (byte i=0;i<dificultad.getNumCasillas();i++) {
						if (contadorRondas==0) {																			//---- Primera Combinacion
						...
						}else if (coloresCorrectos.size()==dificultad.getNumCasillas()&&mapaColores.size()==0){				//---- Si ya se saben los
							...
							if (i<dificultad.getNumCasillas()/2) {																// relativa de al menos 1
								if (mapaColores.containsKey((byte)0))															//
									combinacion.getCombinacion()[i]=new Ficha(mapaColores.get((byte)0));						//
								else																							//
									combinacion.getCombinacion()[i]=new Ficha(mapaColores.get((byte)2));						//
							}else {																								//
								while (mapaColores.containsValue(coloresCorrectos.get(contadorColores)))						//
									contadorColores++;																			//
				...
				case 2:
					...
					if (numCambiosFase2==0) {	// Colocamos combinaciones para ordenar los colores en sus mitades, solo se necesitan 3 combinaciones máximo para adivinar la combinación llegados a este punto
						for (byte i=0;i<mapaColores.size();i++)
							combinacion.getCombinacion()[i]=new Ficha(mapaColores.get(i));
					}else if (numCambiosFase2==1&&tablero.getRondas().getLast().getResultadoByN().equals(new Combinacion(fichasMixtas))) {
						combinacion.getCombinacion()[0]=new Ficha(mapaColores.get((byte)1));
						combinacion.getCombinacion()[1]=new Ficha(mapaColores.get((byte)0));
						combinacion.getCombinacion()[2]=new Ficha(mapaColores.get((byte)2));
						combinacion.getCombinacion()[3]=new Ficha(mapaColores.get((byte)3));
					}else if (numCambiosFase2==1&&tablero.getRondas().getLast().getResultadoByN().equals(new Combinacion(fichasBlancas))) {
						combinacion.getCombinacion()[0]=new Ficha(mapaColores.get((byte)1));
						combinacion.getCombinacion()[1]=new Ficha(mapaColores.get((byte)0));
						combinacion.getCombinacion()[2]=new Ficha(mapaColores.get((byte)3));
						combinacion.getCombinacion()[3]=new Ficha(mapaColores.get((byte)2));
					}else if (numCambiosFase2==2) {
						combinacion.getCombinacion()[0]=new Ficha(mapaColores.get((byte)0));
						combinacion.getCombinacion()[1]=new Ficha(mapaColores.get((byte)1));
						combinacion.getCombinacion()[2]=new Ficha(mapaColores.get((byte)3));
						combinacion.getCombinacion()[3]=new Ficha(mapaColores.get((byte)2));
					}
					break;
	...
	protected void comprobarCombinacion(Ronda ronda) {
		...
		Combinacion combinacionByN_inicial=tablero.getRondas().getFirst().getResultadoByN();
		if (fase==1) {
			...
			if (coloresCorrectos.size()==dificultad.getNumCasillas()) {		// Aquí entra solo cuando ya sabe los colores que son, para así comprobar sus posiciones
				...
				if (mapaColores.size()==dificultad.getNumCasillas())		// Si ya sabe las mitades en las que van todas las fichas correctas, pasa a la segunda fase
					fase++;
			}
		}else if (fase==2) {
